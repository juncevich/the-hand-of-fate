...
class VotePersistenceAdapter(
    ...
    override fun findById(id: UUID): Vote? {
        ...
        val creator = userQueryService.findById(entity.creatorId)
            ?: throw NoSuchElementException("Creator not found for vote $id")
        return entity.toDomain(creator)
    }

    override fun findAllByUserIdOrParticipantEmail(userId: UUID, email: String, pageable: Pageable): Page<Vote> =
        voteJpaRepository.findAllByUserIdOrParticipantEmail(userId, email, pageable).map { entity ->
            val creator = userQueryService.findById(entity.creatorId)
...
class ParticipantPersistenceAdapter(
    ...
    override fun existsByVoteIdAndEmail(voteId: UUID, email: String): Boolean =
        participantJpaRepository.existsByVoteIdAndEmail(voteId, email)

    override fun deleteByVoteIdAndEmail(voteId: UUID, email: String) =
    ...
    override fun findEligibleEmailsForRound(voteId: UUID, round: Int): List<String> =
        participantJpaRepository.findEligibleEmailsForRound(voteId, round)
}
...
class VoteOptionPersistenceAdapter(
    ...
    override fun saveAll(options: List<VoteOption>): List<VoteOption> {
        ...
        return voteOptionJpaRepository.saveAll(
    ...
    override fun countByVoteId(voteId: UUID): Long =
        voteOptionJpaRepository.countByVoteId(voteId)

    override fun deleteByVoteIdAndId(voteId: UUID, optionId: UUID) =
        voteOptionJpaRepository.deleteByVoteIdAndId(voteId, optionId)

    override fun findEligibleOptionsForRound(voteId: UUID, round: Int): List<VoteOption> =
        voteOptionJpaRepository.findEligibleOptionsForRound(voteId, round).map { it.toDomain() }
}
...
class DrawHistoryPersistenceAdapter(
    ...
    override fun save(history: DrawHistory): DrawHistory {
        ...
        val winnerOptionRef = history.winnerOptionId?.let { voteOptionJpaRepository.getReferenceById(it) }
        return drawHistoryJpaRepository.save(
            DrawHistoryJpaEntity(
                id = history.id,
                vote = voteRef,
                winnerEmail = history.winnerEmail,
                winnerDisplayName = history.winnerDisplayName,
                winnerOption = winnerOptionRef,
