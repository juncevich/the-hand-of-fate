...
### @radix-ui/react-avatar `1.1.12 → 1.2.2`
- Патч-релиз `1.2.2` после диапазона `1.2.0`/`1.2.1`; детальный changelog по отдельным версиям не публикуется в npm registry в структурированном виде — конкретные изменения не подтверждены, считать рутинным патч-обновлением

### @radix-ui/react-dialog `1.1.16 → 1.1.19`
- Патч-обновления `1.1.17 → 1.1.18 → 1.1.19`; детали изменений не подтверждены реальным источником в этой сессии

### @radix-ui/react-dropdown-menu `2.1.17 → 2.1.20`
- Патч-обновления `2.1.18 → 2.1.19 → 2.1.20`; детали изменений не подтверждены

### @radix-ui/react-label `2.1.9 → 2.1.11`
- Патч-обновления `2.1.10 → 2.1.11`; детали изменений не подтверждены

### @radix-ui/react-select `2.3.0 → 2.3.3`
- Патч-обновления `2.3.1 → 2.3.2 → 2.3.3`; детали изменений не подтверждены

### @radix-ui/react-separator `1.1.9 → 1.1.11`
- Патч-обновления `1.1.10 → 1.1.11`; детали изменений не подтверждены

### @radix-ui/react-tabs `1.1.14 → 1.1.17`
- Патч-обновления `1.1.15 → 1.1.16 → 1.1.17`; детали изменений не подтверждены

### @radix-ui/react-toast `1.2.16 → 1.2.19`
- Патч-обновления `1.2.17 → 1.2.18 → 1.2.19`; детали изменений не подтверждены

### @radix-ui/react-tooltip `1.2.9 → 1.2.12`
- Патч-обновления `1.2.10 → 1.2.11 → 1.2.12`; детали изменений не подтверждены

> Примечание: для всех пакетов `@radix-ui/*` выше не удалось получить структурированные release notes через npm registry в этой сессии (веб-доступ к GitHub releases не выполнялся) — версии подтверждены как реальные (`npm view ... version`), но конкретное содержание изменений не проверялось и не должно считаться достоверным.

### @tailwindcss/vite `4.3.0 → 4.3.2`, tailwindcss `4.3.0 → 4.3.2`
- Рутинное patch-обновление, синхронно с core-пакетом

### @tanstack/react-query `5.101.0 → 5.101.2`, @tanstack/react-query-devtools `5.101.0 → 5.101.2`
- Рутинное patch-обновление

### @typescript-eslint/eslint-plugin `8.61.0 → 8.63.0`, @typescript-eslint/parser `8.61.0 → 8.63.0`
- Рутинное minor-обновление (`8.62.x → 8.63.0`), пакеты синхронизированы между собой
...
- Рутинное minor-обновление (`10.5.x → 10.6.0`)

### lucide-react `1.17.0 → 1.23.0`
- Рутинное minor-обновление (новые/обновлённые иконки за несколько релизов, точный список не проверялся)

### msw `2.14.6 → 2.14.7`
- Рутинное patch-обновление; транзитивно подтянул исправления, устранённые далее через `npm audit fix` (см. ниже)
...
### Транзитивные зависимости: form-data, undici (через `npm audit fix`)
- `npm audit` после `npm install` показал 2 high-severity уязвимости в транзитивных пакетах `form-data` (CRLF injection, GHSA-hmw2-7cc7-3qxx) и `undici` (несколько TLS/cookie/DoS advisories, включая GHSA-vmh5-mc38-953g). `npm audit fix` обновил их без изменений в `package.json` (транзитивные версии, подтягиваются через `msw`/зависимости). После фикса `npm audit` показал 0 уязвимостей.

### Без изменений (уже на latest на момент проверки)
