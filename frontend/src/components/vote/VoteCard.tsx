...
export function VoteCard({ vote }: { vote: VoteSummary }) {
  return (
    <Link to={`/votes/${vote.id}`} className="block group">
      <div className="glass p-5 hover:border-fate-gold/30 hover:bg-white/[0.06] transition-all duration-300 group-hover:translate-y-[-2px]">

        {/* Header row */}
        <div className="flex items-start justify-between gap-3 mb-3">
          <h3 className="font-semibold text-fate-text group-hover:text-fate-gold transition-colors line-clamp-2">
            {vote.title}
          </h3>
          {vote.isCreator && (
            <Crown className="w-4 h-4 text-fate-gold shrink-0 mt-0.5" />
          )}
        </div>
        ...
        {/* Footer */}
        <div className="flex items-center justify-between text-xs text-fate-muted">
