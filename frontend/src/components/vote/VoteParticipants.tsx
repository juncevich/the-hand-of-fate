      ...
      <div className="space-y-2 mb-4">
        {participants.map((p) => (
          <div key={p.email} className="flex items-center gap-3 py-2">
            <div className="w-8 h-8 rounded-full bg-white/8 flex items-center justify-center text-xs font-medium text-fate-text">
              {(p.displayName ?? p.email)[0].toUpperCase()}
            </div>
            <div className="flex-1">
              {p.displayName && (
                <p className="text-sm font-medium text-fate-text">{p.displayName}</p>
              )}
              <p className="text-xs text-fate-muted">{p.email}</p>
            </div>
            {canEdit && (
              <Button
                variant="ghost"
                ...
                className="text-fate-muted hover:text-red-400 h-6 w-6"
              >
                <X className="w-3 h-3" />
              </Button>
            )}
